...
uint32_t packet_generator::stc_intial_online_sensors(unsigned char* out_buffer, ::std::vector<device_key_t>& keys)
{
    ...
    // Number of sensors
    {
        uint32_t number_of_devices = keys.size();
        memmove(buffer.data() + shift, &number_of_devices, sizeof(uint32_t));
        shift += sizeof(uint32_t);
    }
    for (auto&it : keys)
    {
        auto module = device_manager->get_module_pointer(it);
        auto device = module->opennui_device_instance();

        // Unique key
        {
            uint32_t device_key = it;
    ...
    header.opcode = protocol::opcode::stc_send_all_sensor_info;
